						...
						{gallery.length > 0 ? (
							gallery.map((gallery) => (
								<TableRow key={gallery._id}>
									...
									<TableCell>{gallery.mediatype === "image" ? <Image src={gallery.media || "/placeholder.jpg"} width={200} height={200} alt={gallery.media} className="w-16 h-16 rounded-full object-cover" /> : <video src={gallery.media} controls autoPlay className="w-24 h-32 object-cover" />} </TableCell>
