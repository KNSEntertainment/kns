...
export default function GalleryForm({ handleCloseGalleryModal, galleryToEdit }) {
	...
	useEffect(() => {
		if (galleryToEdit) {
			setFormData({
				mediatype: galleryToEdit.mediatype,
				media: null, // We don't set the file input
				category: galleryToEdit.category,
				alt: galleryToEdit.alt,
			});
		}
	}, [galleryToEdit]);
	...
	return (
		<form onSubmit={handleSubmit} className="space-y-4">
			...
			<div>
				<label htmlFor="mediatype" className="block mb-2 font-bold">
					Media Type
				</label>
				<select id="mediatype" value={formData.mediatype} onChange={(e) => setFormData({ ...formData, mediatype: e.target.value })} className="w-full p-2 border rounded" required>
			...
			<div>
				...
				<input type="file" id="media" onChange={(e) => setFormData({ ...formData, media: e.target.files[0] })} className="w-full p-2 border rounded" accept={formData.mediatype === "image" ? "image/*" : "video/*"} required={!galleryToEdit} />
			</div>
			<div>
